...
impl LuminarRule{
    pub fn match_process(&self, process_util:& LuminarProcessUtil)-> bool{
        process_util.cpu_memory < self.max_cpu_memory && process_util.gpu_memory < self.max_gpu_memory
    }
}
...
pub fn retain(&self,rule:& LuminarProcessUtil)->bool{
    ...
    rule.cpu_usage > self.max_cpu_usage || rule.cpu_memory > self. max_cpu_memory || rule.gpu_usage > self.max_gpu_usage || rule.gpu_memory > self.max_gpu_memory
}
}
...
#[derive(Debug)]
pub struct LuminarManager {
    refresh_interval: f32, // in seconds
...
impl LuminarManager {
    pub fn new(mut users_info: Vec<LuminarUserInfo>,rule_filters: Vec<LuminarRuleFilter>, common_rules : Vec<LuminarRule>, refresh_interval:f32) -> LuminarManager {
        let mut users = vec![];
